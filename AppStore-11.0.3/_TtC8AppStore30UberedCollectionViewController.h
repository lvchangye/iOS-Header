//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Feb  8 2018 12:17:13).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "_TtC8AppStore29StoreCollectionViewController.h"

@class UIView, _TtC11AppStoreKit7Artwork;

@interface _TtC8AppStore30UberedCollectionViewController : _TtC8AppStore29StoreCollectionViewController
{
    // Error parsing type: , name: artworkLoader
    // Error parsing type: , name: uber
    // Error parsing type: , name: statusBarStyle
    // Error parsing type: , name: contentInsetsInvalid
    // Error parsing type: , name: uberHeaderView
    // Error parsing type: , name: uberScrollCoordinator
}

- (CDUnknownBlockType).cxx_destruct;
- (id)initWithCollectionViewLayout:(id)arg1;
- (void)scrollViewDidScroll:(id)arg1;
- (void)scrollViewWillEndDragging:(id)arg1 withVelocity:(struct CGPoint)arg2 targetContentOffset:(struct CGPoint *)arg3;
@property(nonatomic, readonly) double currentUberHeight;
@property(nonatomic, readonly) _Bool hasUber;
- (void)adjustContentInsets;
- (void)invalidateContentInsets;
@property(nonatomic, readonly) long long preferredStatusBarStyle;
@property(nonatomic) long long statusBarStyle; // @synthesize statusBarStyle;
@property(nonatomic, retain) UIView *overlayView;
@property(nonatomic, retain) _TtC11AppStoreKit7Artwork *uber; // @synthesize uber;
- (void)displayWithAccessoryView:(id)arg1;
@property(nonatomic, readonly) _Bool prefersLargeTitleInNavigationBar;
- (void)viewSafeAreaInsetsDidChange;
- (void)viewDidLayoutSubviews;
- (void)viewWillTransitionToSize:(struct CGSize)arg1 withTransitionCoordinator:(id)arg2;
- (void)viewDidDisappear:(_Bool)arg1;
- (void)viewWillDisappear:(_Bool)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (void)viewWillAppear:(_Bool)arg1;
- (void)viewDidLoad;
- (id)initWithCoder:(id)arg1;

@end

